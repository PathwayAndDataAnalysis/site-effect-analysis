...
### Data Formatting
Before you execute the program make sure that your datasets are properly formatted to ensure that the Python script runs properly. 
...
+ If there is more than one site for a site-specific protein, divide each site with a space (" "). Make sure there are no extra spaces anywhere. (ex. "S109s S110s")
...
This is a rough idea of what your phosphoproteomic data should look like: 

<img width="688" height="46" alt="image" src="https://github.com/user-attachments/assets/c9b6ddb9-7c39-4bac-b389-66053c3ec106" />

This is a rough idea of what your proteomic data should look like: 
...
+ The fourth column must consist of the number "1" to imply that the site is an activatory site, "0" to imply that the site effect is unknown, or "-1" to imply that the site is an inhibitory site.
...
This is a rough idea of what your site-effect data should look like.
...
Afterward, the terminal will ask you to provide a name for two directories that will be stored in your current working directory. One of these directories will store your cluster graphs and will have the name you provided and "_Clusters" at the end of the name. The other directory will store your residual graphs and will have the name you provided and "_Residual" at the end of the name. For example, if you type the name "test", then the two directories that will be created in your current working directory will be "test_Clusters" and "test_Residual". If you make a directory with a name that already exists, the script will still run, but no new directory will be created. 

After this, if you see the current gene symbol, correlation coefficient, and p-value on the terminal, then the program is running correctly and graphs should shortly start to generate in the clusters and residual directories. 

## Help
This is general advice to troubleshoot any issues you may have. 
+ Make sure all dependencies are properly installed. 
+ If you are seeing an index error, an issue with interpreting unknown values, or any issue relating to the data, refer to Data Formatting and make sure that the data is properly formatted. 
+ If there is an issue with file or directory creation, make sure that your system has sufficient storage and that you have the proper permissions to create directories and files.
